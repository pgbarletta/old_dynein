C Programa para calcular los modos normales utilizando el metodo de ANM 
      ...
      IMPLICIT NONE
        ...
        ! este es un anmq me mandó tadeo con sólo 2 modficaciones. i
        ! Toma los argumentos de cutoff y structura input de la consola
        ! en vez de leerlo de archivos.
...
C Subroutine calculo
      ...
      subroutine calculo(icont)

      integer ifail,ii
      integer i,icont,slen,flen,long,j,k,m,icontmax
      parameter(icontmax=3000)
      double precision cutoff,xx 
      ...
      do i=1,icont
         ...
         read(51,*) bfactor(i)
         write(60,222) r(i,1),r(i,2),r(i,3),xx,bfactor(i)
      enddo
C****************************************************
C* Calculo de los elementos triangulares superiores *
C*  de la matriz Hess 3Nx3N.- Aclaracion: i.NE.j    *
C****************************************************
           ...
           if(i.ne.j.and.i.lt.j) then
             if(s(i,j).le.cutoff) then
               do m=0,2
                 do k=0,2
                   If(k.eq.m) then
                  hess(i*3-2+m,j*3-2+k)=(r(j,k+1)-r(i,m+1))**2
     $/s(i,j)**2
                              else
                  hess(i*3-2+m,j*3-2+k)=-((r(j,m+1)-r(i,m+1))
...
C************************************************
C* Calculo de los elementos fuera de la diagonal,* 
C*    dentro de las submatrices con i=j.         *
C*************************************************
      do i=1,icont
         do j=1,icont
           if(i.ne.j) then
     ...
     $+((r(j,m+1)-r(i,m+1))*(r(j,k+1)-r(i,k+1)))/s(i,j)**2
                  endif
                enddo
              enddo
             endif            
...
C**************************************************
C*   Calculo de los elementos diagonales dentro   *
C*         de las submatrices i=j.-               *
C**************************************************
     ...
     $+((r(j,k+1)-r(i,k+1))*(r(j,k+1)-r(i,k+1)))/s(i,j)**2
                enddo
             endif
           endif
         enddo
...
C************************************************
C*  Calculo de los elementos transpuestos fuera * 
C*   de la diagonal de las submatrices i.ne.j   *
C************************************************
...
C**********************************************************
C* Calculo de los elem. transp. de las submatrices i.ne.j *
C*     fuera de la diagonal de esa submatriz              *
C**********************************************************  
                       hess(i*3-2+m,j*3-2+k)=hess(j*3-2+k,i*3-2+m)
                      endif
                   enddo
...
C*********************
      ...
      do i=1,icont*3
         write(10,334) (corr(i,j),j=1,icont*3)
      enddo 
      close(10)
...
C*******************************************************
C*  CORRELACION ENTRE bfactor teorico y experimental   *
C*******************************************************
C*     AA es la suma de los (Xi-Xprom)(Yi-Yprom)
C*     AB es la suma de los (Xi-Xprom) al cuadrado
C*     AC es la suma de los (Yi-Yprom) al cuadrado
      ...
      do i=1,icont
          write(10,444)'listabfactor',i,bfacteor(i)*SUMBFAC/SUMCORR
     $,bfactor(i),CORSTD 
      enddo
      close(10)

      open(10,file='bfactorcorr')
      write(10,335) CORSTD
      close(10)
...
111   FORMAT(F6.2,1x)    
222   FORMAT(3(1x,F7.3),1x,F6.2,1x,F6.2,1x) 
333   FORMAT(10000(1x,F9.6)) 
334   FORMAT(10000(1x,F9.6))
444   FORMAT(A,i5,1x,F12.2,1x,F7.3,1x,F7.4)
555   FORMAT(1(1x,F7.3))
