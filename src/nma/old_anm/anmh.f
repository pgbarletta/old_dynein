...
! Este código es el original q me mandó Tadeo en el mail "anm código contactos"
! Buscar ese mail p/ conseguir el original posta. De cualquier manera, las 
! modificaciones son sólamente de input. Los rtdos deberían ser idénticos.
...
c      integer sa(icont3),sb(icont3)
      ...
      character*1000 card
      character*30 cutoffc, in_h


      call getarg(2, in_h)
      call getarg(3, cutoffc)
      read (cutoffc, *) cutoff

      open(50,file='temp')
      open(51,file='temp1')
c      open(60,file='comprobar')
      ...
      enddo
       ...
       if ( in_h /= 'none' ) then
            ...
            do
                i = i + 1
                read(11, 91, iostat= ierr) a(i), b(i)
                if ( ierr/= 0 ) exit
            enddo
...
*********************
      ...
      do i=1,icont
          write(29,445) i,bfacteor(i)*SUMBFAC/SUMCORR
      enddo
      close(29)
...
*     AC es la suma de los (Yi-Yprom) al cuadrado
      ...
      do i=1,icont
          write(10,444) i,bfacteor(i)*SUMBFAC/SUMCORR,bfactor(i)
     $,CORSTD 
      enddo
      close(10)
